...
import { INCIL_BOOKS } from "@/constants/content";
import { useReligious } from "@/context/ReligiousContext";

export default function IncilTab() {
  ...
  if (!isPremium) {
    return (
      <PremiumLockOverlay
        contentName="İncil"
        description="İncil'in tam metnine, Türkçe tercümeleri ve tefsirleriyle birlikte erişmek için Premium üyeliğe geçin."
      />
    );
  }
  ...
  const openBook = (id: string) => {
    ...
    router.push(`/scripture/incil/${id}`);
  };
    ...
    >
      <Animated.View entering={FadeInDown.duration(500)}>
        <View style={styles.header}>
          ...
          <Text style={styles.appLabel}>Yeni Ahit</Text>
          <Text style={styles.title}>İncil</Text>
          <Text style={styles.subtitle}>
            {INCIL_BOOKS.length} kitap · Reklamsız okuma
          </Text>
        </View>
      </Animated.View>

      <View style={styles.list}>
        {INCIL_BOOKS.map((book, idx) => {
